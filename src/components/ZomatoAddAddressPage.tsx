...
const ZomatoAddAddressPage: React.FC<ZomatoAddAddressPageProps> = ({
  ...
  const initializeMap = async () => {
    try {
      ...
      if (mapId && mapId.trim() !== "") {
        ...
        console.log("🎨 Map styles will be controlled via Google Cloud Console when using Map ID");
      } else {
  ...
  const updateMapLocation = useCallback(
    (coordinates: Coordinates) => {
      ...
      if (marker) {
        if (markerLibrary?.AdvancedMarkerElement && marker instanceof markerLibrary.AdvancedMarkerElement) {
          marker.map = null;
