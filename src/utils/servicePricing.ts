...
const serviceCategoryMap = new Map<string, ServicePriceInfo[]>();
...
laundryServices.forEach((service) => {
  ...
  // Add to category map
  if (!serviceCategoryMap.has(service.category)) {
    serviceCategoryMap.set(service.category, []);
  }
  serviceCategoryMap.get(service.category)!.push(priceInfo);
});
 ...
 */
export const searchServices = (query: string): ServicePriceInfo[] => {
  if (!query) return [];

  const results: ServicePriceInfo[] = [];
  const lowerQuery = query.toLowerCase();

  for (const [key, value] of serviceNameMap.entries()) {
    if (
      key.includes(lowerQuery) &&
      !results.find((r) => r.name === value.name)
    ) {
      results.push(value);
    }
  }
